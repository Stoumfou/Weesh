...
exports.sites = {
	...
	asos: function (path) {
		...
		productSelector.product_id = 'div.product-code > span';
	...
	bestbuy: function (path) {
		...
		productSelector.ean = '';
		productSelector.product_id = '##sku-value';
		productSelector.image = xDelay('meta[property="og:image"]@content');
		productSelector.isbn = '';
		productSelector.model = '';
		productSelector.original_price = x('.regular-price');
		productSelector.price = '.item-price';
		productSelector.sales_price = '';
	...
	castorama: function (path) {
		...
		productSelector.product_id = '.refNum';
	...
	darty: function (path) {
		...
		productSelector.product_id = '#main > div@data-codic'
		productSelector.title = 'meta[property="og:title"]@content';
		productSelector.ean = '#flix-minisite@data-flix-ean';
		...
		productSelector.price = '#darty_product_4215621 > div.product_column.product_right > div.product_infos.clearfix > div.product_metas > meta:nth-child(5)';
		...
		productSelector.description = 'meta[property="og:description"]@content';
		productSelector.brand = '#darty_product_brand';
		productSelector.image = xDelay('meta[property="og:image"]@content');
	...
	eanfind: function (path) {

		var productSelector = this.base();


			productSelector.sellerArray = x(['#priceCountry > div.active > div > table > tbody > tr > td.store > div > img@title']);
			productSelector.priceArray = x(['#priceCountry > div.active > div > table > tbody > tr > td.center > span']);
			productSelector.dateArray = x(['#priceCountry > div.active > div > table > tbody > tr > td.infos > span.last-update']);
			productSelector.ean = '#infos > ul > li > div.span10';

		var pageURL = 'https://www.eanfind.fr/chercher/' + path;
	...
	fnac: function (path) {
		...
		productSelector.product_id = 'body@data-prid';
		productSelector.title = 'meta[property="og:title"]@content';
		productSelector.original_price = '.oldPrice';
	...
	grosbill: function (path) {
		...
		productSelector.product_id = '#PopinDiv@data-product';
	...
	laredoute: function (path) {
		...
		productSelector.product_id = '#pdpFRdivMain@data-productid';
		productSelector.title = 'meta[property="og:title"]@content';
	...
	ldlc: function (path) {
		...
		productSelector.details = x(['.allparams > td ']);
	...
	rueducommerce: function(path) { 
		...
		productSelector.product_id = '#BVRRContainer > div > div@data-product-id'; 
		productSelector.isbn = 'meta[property="og:isbn"]@content';
	...
	tesco: function (path) {
		...
		productSelector.product_id = 'meta[property="og:upc"]@content';
	...
	ubaldi: function(path) { 
		...
		productSelector.product_id = '#wc-power-page@data-product-id'; 
		...
		productSelector.price = '.rebours-clignote@data-prix-vente';
	...
	if (this.sites[opts.site]) {
		...
		x(lookup.page, lookup.selectors)
			(function (err, obj) {
				...
				if (_obj.dateArray) {
					var dateArray = [];
					_obj.dateArray.forEach(function (element, index, array) {
						dateArray.push(element
							.replace(/(?:\t|\r|\n)/g, '')
							.replace(/ +(?= )/g, '')
							.replace(' Prix modifié :', '')
							.replace('.', '')
							.replace('.', ''));
					});
					_obj.dateArray = dateArray;
				}

				if (_obj.priceArray) {
					var priceArray = [];
					_obj.priceArray.forEach(function (element, index, array) {
						priceArray.push(element
							.replace(',', '.'));
					});
					_obj.priceArray = priceArray;
				}
			if(_obj.product_id) _obj.product_id = _obj.product_id.trim();
			// RAJOUTER CONDITION POUR CASTORAMA 
			if(_obj.product_id) _obj.product_id = _obj.product_id.trim()
					.replace('Réf :', '');
			// RAJOUTER CONDITION POUR LDLC
			if(_obj.product_id) _obj.product_id = _obj.product_id.trim()
					.replace('.html', '');
			...
			if(_obj.original_price) _obj.original_price = _obj.original_price.trim().replace('(Reg. ', '').replace(')', '').replace('*', ''); 
			// RAJOUTER CONDITION POUR DESCRIPTIONS AMAZON
			if(_obj.description){
					_obj.description = _obj.description.trim()
					.replace(/(?:\t|\r|\n)/g, '')
					.replace(/ +(?= )/g,'');	
					_obj.description = _obj.description.substring(0, 199);
					}
			if(_obj.title){
				_obj.title = _obj.title.trim();
				_obj.title = _obj.title.substring(0, 199);
			}
			if(_obj.price) _obj.price = _obj.price.trim().replace('*', '').replace('â¬', '');
			if(_obj.sale_price) _obj.sale_price = _obj.sale_price.trim().replace('*', '');
		...
		var selectors = {
			product_id: '[itemprop="product-id"]',
...
exports.parseURL = function (url, callback) {
	...
	else if (parse.host == 'www.eanfind.fr') {
		this.scraper({
			site: 'eanfind',
			product_id: parse.pathname.replace('/','') + parse.query
		}, callback);
